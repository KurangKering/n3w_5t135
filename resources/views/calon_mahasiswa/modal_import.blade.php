...
<div id="modal-import" style="display: none" style="z-index: 999999999">
  <div class="box">
    <form id="form-import" action="{{ route('calon_mahasiswa.import') }}" method="POST" class="" enctype="multipart/form-data">
      <div class="box-body">
...
<script>
  ...
  $("#modal-import").iziModal({
    title: 'Import Data Calon Mahasiswa',
    subtitle: '',
    headerColor: '#676a6c',
    //   background: null,
    //   theme: '',  
  ...
  var show_modal_import = function(id)
  {
    ...
    .then(res => {
      response = res.data;
      set_modal_data_import(response);
      $('#modal-import').iziModal('open');
    })
    .catch();
  ...
  var set_modal_data_import = function(data)
  {
    $("#nama").text(data.nama);
    $("#nisn").text(data.nisn);
  ...
  $("#form-import").submit(function(e) {
    ...
    axios.post('{{ route('calon_mahasiswa.import') }}', formData)
    .then(response => {
      resp = response.data;
